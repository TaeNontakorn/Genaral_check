...
def compare_documents(text_a: str, text_b: str, name_a: str, name_b: str, gemini_client: genai.Client) -> str:
    prompt = f"""
        ...
        กฏหลัก:
        1. ระบุข้อความแตกต่างกันทั้งหมดใน text_a และ text_b ทั้งหมดเช่น เจอข้อมูลที่ต่างกัน 10 รายการ ให้ระบุทั้ง 10 รายการออกมา
        2. หากมีข้อมูลที่เหมือนกันแต่มีความแตกต่างกัน เช่น ตัวเลขไม่ตรงกัน หรือคำที่คล้ายกันแต่สะกดต่างกัน ให้ระบุออกมาเช่นกัน
        3. หากมีความแตกต่างในรูปแบบการนำเสนอ เช่น การจัดวางตาราง หรือการใช้หัวข้อ ให้ระบุออกมาเช่นกัน
        4. หากเอกสารทั้งสองเหมือนกันทุกประการ ให้ระบุว่า "ไม่พบความแตกต่าง"
        5. ถ้าเป็นชื่อคนให้ดูชื่อว่าชื่อถูกต้องไหมไม่ต้องนับคำนำหน้าหรือคำนำท้าย เช่น "นาย" "นางสาว" "นาง" "บริษัท" "ห้างหุ้นส่วนจำกัด" ให้ดูแค่ชื่อจริงกับนามสกุลว่าตรงกันไหม แต่ถ้าเป็นชื่อหน่วยงานให้ดูทั้งชื่อเลยว่าตรงกันไหม เช่น "บริษัท ABC จำกัด" กับ "บริษัท ABC จำกัด" ถือว่าเหมือนกัน แต่ถ้าเป็น "บริษัท ABC จำกัด" กับ "บริษัท ABC จำกัด มหาชน" ถือว่าต่างกัน เพราะเป็นคนละหน่วยงาน ถึงแม้ชื่อย่อจะเหมือนกันก็ตาม
